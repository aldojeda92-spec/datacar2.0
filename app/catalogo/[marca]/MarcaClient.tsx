// app/catalogo/[marca]/MarcaClient.tsx
'use client';
...
interface MarcaClientProps {
  ...
  initialModels?: ModelData[] | null;
}

export default function MarcaClient({ marcaSlug, initialBrandName = '', initialModels = null }: MarcaClientProps) {
  const [brandName, setBrandName] = useState<string>(
    initialBrandName || marcaSlug.replace(/-/g, ' ').toUpperCase()
  );
  const [models, setModels] = useState<ModelData[]>(initialModels ?? []);
  const [loading, setLoading] = useState(initialModels === null);

  // ==========================================
  // MOTOR DE CONSULTA (FIREBASE) — solo si el servidor no entregó los modelos
  // (fallback ante error de red server-side o navegación SPA).
  // ==========================================
  useEffect(() => {
    if (initialModels !== null) return;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col items-center justify-center">
        ...
        <p className="font-bold text-[#0A1F33] tracking-widest uppercase text-[10px]" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
    ...
    <main className="min-h-screen bg-[#F8F9FA] text-[#3A3A3C] font-sans flex flex-col">
      ...
      <header className="bg-[#FFFFFF] border-b border-[#C0C0C0] py-8 px-4 md:px-8">
        <div className="max-w-[1200px] mx-auto">
          {/* Breadcrumbs */}
          <div className="text-[10px] text-[#C0C0C0] uppercase tracking-widest font-bold mb-4 flex gap-2 items-center" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
          ...
          <h1 className="font-black text-3xl md:text-4xl text-[#0A1F33] uppercase tracking-wide" style={{ fontFamily: 'var(--font-montserrat), sans-serif' }}>
            Catálogo de autos 0km <span className="text-[#00BFFF]">{brandName}</span>
          </h1>
          <p className="text-xs text-[#3A3A3C] mt-2 font-medium" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
              ...
              {models.map((model, index) => (
                <article key={model.id} className="bg-[#FFFFFF] border border-[#C0C0C0] flex flex-col hover:border-[#0A1F33] transition-colors group cursor-pointer shadow-none rounded-none">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h2 className="font-black text-xl text-[#0A1F33] uppercase leading-tight" style={{ fontFamily: 'var(--font-montserrat), sans-serif' }}>
                      <span className="text-[#3A3A3C] font-medium text-sm block mb-1">{brandName}</span>
                      {model.name}
                    </h2>
                    
                    <p className="text-[10px] text-[#C0C0C0] uppercase tracking-widest font-bold mt-2" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
                    ...
                    <div className="mt-auto pt-6">
                      ...
                      <p className="font-black text-2xl text-[#0A1F33] tracking-tight" style={{ fontFamily: 'var(--font-montserrat), sans-serif' }}>
      ...
      <footer className="w-full bg-[#0A1F33] border-t-4 border-[#00BFFF] py-12 px-4 md:px-8 text-center mt-auto shrink-0">
        <div className="max-w-[1200px] mx-auto flex flex-col items-center">
          <div className="font-black text-2xl tracking-widest text-[#FFFFFF] uppercase mb-6" style={{ fontFamily: 'var(--font-montserrat), sans-serif' }}>
