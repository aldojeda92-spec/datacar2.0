...
export default function ModeloDetailPage() {
  ...
  // Formularios adaptados a las nuevas reglas (Celular 09XXXXX, Email opcional)
  ...
  // ==========================================
  // LÓGICAS OPERATIVAS (CON REGLAS DE LEAD)
  // ==========================================
  const handleConsultar = async (e: React.FormEvent) => {
    e.preventDefault(); 
    
    // Validación estricta de celular Paraguay (10 dígitos, empieza con 09)
    const phoneRegex = /^09\d{8}$/;
    if (!phoneRegex.test(leadForm.telefono)) {
      setFeedback({ type: 'error', message: 'El celular debe tener 10 dígitos y empezar con 09. Ej: 0981234567' });
      return;
    }

    setFeedback({ type: '', message: 'Procesando consulta...' });
    try {
      await addDoc(collection(db, 'leads'), {
        nombre: leadForm.nombre,
        telefono: leadForm.telefono,
        email: leadForm.email || 'No proporcionado', // Opcional
        vehiculo: `${brand?.name} ${model?.name}`,
        concesionaria_destino: baseVersion?.concesionaria || brand?.name || 'A designar', // Enrutamiento inteligente
        origen: 'Ficha Modelo', 
        estado: 'Nuevo', 
        createdAt: serverTimestamp()
      });
      setFeedback({ type: 'success', message: 'Consulta enviada. Un asesor se comunicará contigo.' });
      setTimeout(() => { setShowConsultar(false); setFeedback({ type:'', message:'' }); }, 3000);
    } catch (error) { 
      setFeedback({ type: 'error', message: 'Error de red. Intenta nuevamente.' }); 
    }
  };
  ...
  return (
    <main className="min-h-screen bg-[#FFFFFF] text-[#3A3A3C] font-sans pb-32">
        ...
        <div className="bg-[#FFFFFF] border border-[#C0C0C0] flex flex-col md:flex-row">
          ...
          <div className="md:w-2/5 p-10 flex flex-col justify-center bg-[#F8F9FA]">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => setShowConsultar(true)} className="w-full bg-[#00BFFF] hover:bg-[#0A1F33] text-[#FFFFFF] font-bold text-xs uppercase tracking-widest py-4 transition-colors flex items-center justify-center gap-2">
              ...
              <button onClick={() => setShowCalcular(true)} className="w-full bg-[#FFFFFF] border border-[#0A1F33] text-[#0A1F33] hover:bg-[#0A1F33] hover:text-[#FFFFFF] font-bold text-xs uppercase tracking-widest py-4 transition-colors flex items-center justify-center gap-2">
              ...
              <div key={ver.id} className="bg-[#FFFFFF] border border-[#C0C0C0] p-6 flex flex-col lg:flex-row items-center gap-6 hover:border-[#0A1F33] transition-colors">
                ...
                <div className="lg:w-1/2 flex flex-col sm:flex-row items-center justify-end gap-6 w-full border-t lg:border-t-0 lg:border-l border-[#C0C0C0] pt-6 lg:pt-0 lg:pl-6">
                  ...
                  <div className="flex flex-col gap-2 w-full sm:w-auto">
                    <button onClick={() => setShowConsultar(true)} className="w-full sm:w-40 bg-[#00BFFF] hover:bg-[#0A1F33] text-[#FFFFFF] font-bold text-[10px] uppercase tracking-widest py-3 transition-colors">Consultar</button>
                    <div className="flex gap-2">
                      <Link href={`/catalogo/${marcaSlug}/${modeloSlug}/${ver.id}`} className="flex-1 sm:w-20 text-center bg-[#FFFFFF] border border-[#0A1F33] text-[#0A1F33] hover:bg-[#0A1F33] hover:text-[#FFFFFF] font-bold text-[9px] uppercase tracking-widest py-2.5 transition-colors">Detalles</Link>
                      <button onClick={() => handleComparar(ver)} className={`flex-1 sm:w-24 border font-bold text-[9px] uppercase tracking-widest py-2.5 transition-colors flex justify-center items-center gap-1 ${isInCompare ? 'bg-[#E6F4EA] border-[#1E8E3E] text-[#1E8E3E]' : 'bg-[#F8F9FA] border-[#C0C0C0] text-[#3A3A3C] hover:border-[#00BFFF] hover:text-[#00BFFF]'}`}>
                  ...
                  {baseVersion.features.seguridad_standard && baseVersion.features.seguridad_standard.length > 0 ? (
                    <>
                      ...
                      {baseVersion.features.seguridad_standard.length > 4 && (
                        <button onClick={() => setShowAllSecurity(!showAllSecurity)} className="text-left mt-2 text-[9px] font-bold text-[#0A1F33] hover:text-[#00BFFF] uppercase tracking-widest transition-colors">
                  ...
                  {baseVersion.features.confort_conveniencia && baseVersion.features.confort_conveniencia.length > 0 && (
                    <>
                      ...
                      {baseVersion.features.confort_conveniencia.length > 3 && (
                        <button onClick={() => setShowAllConfort(!showAllConfort)} className="text-left mt-2 text-[9px] font-bold text-[#0A1F33] hover:text-[#00BFFF] uppercase tracking-widest transition-colors">
      ...
      {/* ==========================================
          MODALES DE CONVERSIÓN CON NUEVAS REGLAS
          ========================================== */}
      
      {showConsultar && (
        <div className="fixed inset-0 bg-[#0A1F33]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#FFFFFF] p-8 max-w-md w-full border-t-4 border-[#00BFFF]">
            <button onClick={() => setShowConsultar(false)} className="absolute top-4 right-4 text-[#C0C0C0] hover:text-[#D93025] font-black">✕</button>
            <h3 className="font-black text-2xl text-[#0A1F33] uppercase mb-1" style={{ fontFamily: 'Montserrat, sans-serif' }}>Solicitar Asesoría</h3>
            <p className="text-[10px] font-bold text-[#C0C0C0] uppercase tracking-widest mb-6 line-clamp-1">Vehículo: {model.name}</p>
            <form onSubmit={handleConsultar} className="flex flex-col gap-4">
              <div>
                <label className="text-[10px] font-bold text-[#3A3A3C] uppercase tracking-widest block mb-1">Nombre Completo <span className="text-[#D93025]">*</span></label>
                <input type="text" className="w-full border border-[#C0C0C0] p-3 text-xs focus:outline-none focus:border-[#0A1F33] bg-[#F8F9FA]" required value={leadForm.nombre} onChange={e=>setLeadForm({...leadForm, nombre: e.target.value})} />
              </div>
              <div>
                <label className="text-[10px] font-bold text-[#3A3A3C] uppercase tracking-widest block mb-1">Celular (Empieza con 09) <span className="text-[#D93025]">*</span></label>
                <input type="tel" minLength={10} maxLength={10} placeholder="Ej: 0981234567" className="w-full border border-[#C0C0C0] p-3 text-xs focus:outline-none focus:border-[#0A1F33] bg-[#F8F9FA]" required value={leadForm.telefono} onChange={e=>setLeadForm({...leadForm, telefono: e.target.value})} />
              </div>
              <div>
                ...
                <input type="email" placeholder="Para recibir presupuestos en PDF" className="w-full border border-[#C0C0C0] p-3 text-xs focus:outline-none focus:border-[#0A1F33] bg-[#F8F9FA]" value={leadForm.email} onChange={e=>setLeadForm({...leadForm, email: e.target.value})} />
              </div>
              <button type="submit" className="w-full bg-[#0A1F33] hover:bg-[#00BFFF] text-[#FFFFFF] font-bold text-xs uppercase tracking-widest py-4 transition-colors mt-2">Enviar Solicitud</button>
              {feedback.message && <p className={`text-[10px] text-center font-bold uppercase tracking-widest mt-2 ${feedback.type==='error'?'text-[#D93025]':'text-[#1E8E3E]'}`}>{feedback.message}</p>}
            </form>
          </div>
        </div>
      )}
      ...
      {showCalcular && (
        <div className="fixed inset-0 bg-[#0A1F33]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#FFFFFF] p-8 max-w-md w-full border-t-4 border-[#0A1F33]">
            <button onClick={() => { setShowCalcular(false); setCuotaCalculada(null); setFeedback({type:'', message:''}); }} className="absolute top-4 right-4 text-[#C0C0C0] hover:text-[#D93025] font-black">✕</button>
            ...
            <form onSubmit={handleCalcular} className="flex flex-col gap-4">
              <div><label className="text-[10px] font-bold text-[#3A3A3C] uppercase tracking-widest block mb-2">Entrega Inicial (USD)</label><input type="number" placeholder="Ej: 10000" className="w-full border border-[#C0C0C0] p-3 text-xs focus:outline-none focus:border-[#0A1F33] bg-[#F8F9FA]" required value={calcForm.entrega} onChange={e=>setCalcForm({...calcForm, entrega: e.target.value})} /></div>
              <div>
                <label className="text-[10px] font-bold text-[#3A3A3C] uppercase tracking-widest block mb-2">Plazo de Financiación</label>
                <select className="w-full border border-[#C0C0C0] p-3 text-xs focus:outline-none focus:border-[#0A1F33] bg-[#FFFFFF]" value={calcForm.plazo} onChange={e=>setCalcForm({...calcForm, plazo: e.target.value})}>
              ...
              <button type="submit" className="w-full bg-[#00BFFF] hover:bg-[#0A1F33] text-[#FFFFFF] font-bold text-xs uppercase tracking-widest py-4 transition-colors mt-2">Generar Simulación</button>
            ...
            <div className="flex gap-2 flex-wrap justify-center">
              {compareList.map(v => (
                <div key={v.id} className="bg-[#FFFFFF]/10 border border-[#FFFFFF]/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest flex items-center gap-3">
                  ...
                  <button onClick={() => handleRemoveCompare(v.id)} className="text-[#C0C0C0] hover:text-[#D93025] transition-colors" title="Quitar"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                </div>
              ))}
            </div>
          </div>
          <div className="flex gap-4 w-full md:w-auto justify-center">
            <button onClick={clearCompare} className="text-[10px] font-bold uppercase tracking-widest text-[#C0C0C0] hover:text-[#FFFFFF] transition-colors">Limpiar</button>
            <Link href="/comparador" className="bg-[#00BFFF] hover:bg-[#FFFFFF] hover:text-[#0A1F33] text-[#FFFFFF] text-[10px] font-bold uppercase tracking-widest px-8 py-3 transition-colors text-center">Comparar Ahora</Link>
          </div>
        </div>
      )}
          ...
          <div className="md:w-2/3 w-full flex flex-col border-t border-[#C0C0C0]">
            {faqs.map((faq, index) => (
              <div key={index} className="border-b border-[#C0C0C0] py-6">
                <button onClick={() => setOpenFaq(openFaq === index ? null : index)} className="w-full flex justify-between items-center text-left focus:outline-none group">
            ...
            <div className="md:w-1/2 w-full max-w-lg">
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-0 w-full">
                <input 
                  ...
                  className="flex-grow p-4 text-sm border-none focus:outline-none bg-[#FFFFFF] text-[#3A3A3C] font-medium" 
                />
                <button type="submit" disabled={subStatus === 'loading'} className="bg-[#00BFFF] hover:bg-[#FFFFFF] text-[#0A1F33] font-bold text-xs uppercase tracking-widest px-8 py-4 transition-colors disabled:opacity-50">
